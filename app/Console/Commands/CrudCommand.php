...
class CrudCommand extends Command {
  ...
  public function handle() {
    ...
    $this->routeName = ($routeGroup) ? $routeGroup . '/' . Str::snake($name, '-') : 'dashboard/' . Str::snake($name, '-');
    $perPage = intval($this->option('pagination'));

    $StoreRequestNamespace = ($this->option('storerequest-namespace')) ? $this->option('storerequest-namespace') . '\\' : 'Http/Requests/Backend/Main/' . Str::singular($name) . '//';
    $UpdateRequestNamespace = ($this->option('updaterequest-namespace')) ? $this->option('updaterequest-namespace') . '\\' : 'Http/Requests/Backend/Main/' . Str::singular($name) . '//';

    $controllerNamespace = ($this->option('controller-namespace')) ? $this->option('controller-namespace') . '\\' : 'Backend/Main/';
    $modelNamespace = ($this->option('model-namespace')) ? trim($this->option('model-namespace')) . '\\' : 'Models\\Backend\\Main\\';
    ...
    $viewPath = ($this->option('view-path')) ? $this->option('view-path') . '\\' : 'pages/backend/main';
    ...
    $formHelper = ($this->option('form-helper')) ? $this->option('form-helper') : 'html';
    $softDeletes = $this->option('soft-deletes');

    // NEW
    $this->call('crud:storerequest', ['name' => $StoreRequestNamespace . Str::singular($modelName) . 'StoreRequest', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    $this->call('crud:updaterequest', ['name' => $UpdateRequestNamespace . Str::singular($modelName) . 'UpdateRequest', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);

    $this->call('crud:controller', ['name' => $controllerNamespace . Str::singular($modelName) . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    $this->call('crud:model', ['name' => $modelNamespace . Str::singular($modelName), '--fillable' => $fillable, '--table' => $tableName, '--pk' => $primaryKey, '--relationships' => $relationships, '--soft-deletes' => $softDeletes]);
    ...
    if (file_exists($routeFile) && (strtolower($this->option('route')) === 'yes')) {
      $this->controller = ($controllerNamespace != '') ? 'Backend\\Main\\' . Str::singular($name) . 'Controller' : $name . 'Controller';
      $this->routeController = Str::singular($modelName) . "Controller";
  ...
  */
  protected function addRoutes()
  {
    return [
      "Route::group([",
      " 'as' => 'dummy.',",
      " 'prefix' => '" . $this->routeName . "',",
      " 'namespace' => 'Backend\Main',",
      "], function(){",
        " Route::get('status-done/{id}', '" . $this->routeController. "@status_done')->name('status-done');",
        " Route::get('status-pending/{id}', '" . $this->routeController. "@status_pending')->name('status-pending');",
        " Route::get('enable/{id}', '" . $this->routeController. "@enable')->name('enable');",
        " Route::get('disable/{id}', '" . $this->routeController. "@disable')->name('disable');",
        " Route::get('status/{id}/{slug}', '" . $this->routeController. "@status')->name('status');",
        " Route::get('delete/{id}', '" . $this->routeController. "@delete')->name('delete');",
        " Route::get('deleteall', '" . $this->routeController. "@deleteall')->name('deleteall');",
        " Route::resource('/', '" . $this->routeController . "')->parameters(['' => 'id']);",
        "});",
      ];
    }
